...
cursor = list(scrp_listen.find({"Planungsleistungen": {"$in": ["Stadtplanung"]}}))


automatischeBranche = list(client_239["staticdata"]["automatischeBranche"].find({}))
cache = {v["branche_id"]: v["branche"] for v in automatischeBranche if "branche_id" in v.keys()}
...
zf_239.update_many(
    {"ZFID": {"$in": zfids}}, {"$addToSet": {"Meta.BranchenDetails.Extern": branche}}
)
